...
void mmu_init(void) {
    LOG_INFO("MMU: Configuring MAIR_EL1...");
    ...
    LOG_INFO("MMU: Building initial Page Tables (Identity Map)...");
    for (int i = 0; i < 512; i++) {
        l1_table[i] = 0;
        high_l1_table[i] = 0;
    }
    ...
    l1_table[0] = 0x00000000 | PTE_VALID | PTE_BLOCK | PTE_AF | PTE_ATTRINDX(MT_DEVICE_nGnRnE);
    ...
    for (uint64_t i = 1; i < 512; i++) {
        l1_table[i] = (i << 30) | PTE_VALID | PTE_BLOCK | PTE_AF |
                      PTE_ATTRINDX(MT_NORMAL) | PTE_SH_INNER;
    }
    __asm__ volatile("msr ttbr0_el1, %0" : : "r"((uint64_t)l1_table));
    __asm__ volatile("msr ttbr1_el1, %0" : : "r"((uint64_t)high_l1_table));

    LOG_INFO("MMU: Configuring TCR_EL1...");
    ...
    LOG_INFO("MMU: Enabling SCTLR_EL1.M...");
